...
-define(MOD_ARCHIVE2_SCHEMA,
        [#table{name = archive_jid_prefs,
                ...
                types = [string, string, string, string, bool, enum, integer,
                         enum],
                enums = [false, approve, concede, forbid, oppose, prefer,
                         require, body, message, stream],
                keys = 5},
         #table{name = archive_global_prefs,
                fields = record_info(fields, archive_global_prefs),
                types = [string, enum, integer, enum, enum, enum, enum, bool],
                enums = [false, approve, concede, forbid, oppose, prefer,
                         require, body, message, stream],
                keys = 1},
